...
from awbot.utils.exceptions import AppCommandPermissionLevelError, PermissionLevelError

db = DatabaseController().guild_config

T = TypeVar("T", bound=commands.Context[awbot] | discord.Interaction)
...
async def has_permission(
    source: commands.Context[awbot] | discord.Interaction,
    lower_bound: int,
...
def permission_check(
    level: int,
    or_higher: bool = True,
) -> Callable[[commands.Context[awbot] | discord.Interaction], Coroutine[Any, Any, bool]]:
    ...
    async def predicate(ctx: commands.Context[awbot] | discord.Interaction) -> bool:
        """
        Check if the user has the required permission level.

        Parameters
        ----------
        ctx : commands.Context[awbot] | discord.Interaction
            The context or interaction to check permissions for.
...
def has_pl(level: int, or_higher: bool = True):
    ...
    async def wrapper(ctx: commands.Context[awbot]) -> bool:
        """
        Check if the user has the required permission level.
