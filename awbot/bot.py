...
from awbot.cog_loader import CogLoader
from awbot.database.client import db
from awbot.utils.banner import create_banner
from awbot.utils.config import Config
from awbot.utils.emoji import EmojiManager
from awbot.utils.env import is_dev_mode
from awbot.utils.sentry import start_span, start_transaction
...
class awbot(commands.Bot):
    """
    Main bot class for awbot, extending discord.py's Bot.

    Handles setup, cog loading, error handling, Sentry tracing, and resource cleanup.
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initialize the awbot bot and start setup process."""
    ...
    async def _setup_hot_reload(self) -> None:
        ...
        if not self._hot_reload_loaded and "awbot.utils.hot_reload" not in self.extensions:
            with start_span("bot.setup_hot_reload", "Setting up hot reload system"):
                try:
                    await self.load_extension("awbot.utils.hot_reload")
