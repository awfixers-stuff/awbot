...
from awbot.utils.flags import CaseModifyFlags, CasesViewFlags
...
class Cases(ModerationCogBase):
    ...
    @commands.guild_only()
    @checks.has_pl(2)
    async def cases(self, ctx: commands.Context[awbot], case_number: str | None = None) -> None:
        """
    ...
    def _create_case_list_embed(
        ...
        for case in cases:
            ...
            type_emoji = self.bot.emoji_manager.get(
                CASE_TYPE_EMOJI_MAP.get(case.case_type, "awbot_error"),
            )
            action_emoji = self.bot.emoji_manager.get(
                CASE_ACTION_MAP.get(case.case_type, "awbot_error"),
            )
            ...
            case_date = (
                ...
                else f"{self.bot.emoji_manager.get('awbot_error')}"
            )
