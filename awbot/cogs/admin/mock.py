...
from awbot.handlers.error import ERROR_CONFIG_MAP
...
class ErrorTestRegistry:
    ...
    def _build_test_registry(self) -> None:
        ...
        for error_type in ERROR_CONFIG_MAP:
            if error_type.__module__.startswith("discord.app_commands"):
                ...
                # Custom errors from awbot modules
    ...
    def _add_custom_test(self, error_type: type[Exception]) -> None:
        """Add custom error test cases for awbot module errors."""
        name = error_type.__name__
        ...
        self.tests[name] = ErrorTestDefinition(
            ...
            realistic_args,
            {},
            f"Custom awbot {name.lower().replace('error', ' error')}",
            "Custom",
        )

    def _get_realistic_custom_args(self, error_name: str) -> tuple[Any, ...]:
        """Get realistic arguments for custom awbot errors."""
        error_name_lower = error_name.lower()
...
class Mock(commands.Cog):
    ...
    async def _send_test_summary(self, ctx: commands.Context[awbot]) -> None:
    ...
    async def mock(self, ctx: commands.Context[awbot]) -> None:
        """
    ...
    @checks.has_pl(level=8)
    async def mock_error(self, ctx: commands.Context[awbot], category: str, error_name: str | None = None) -> None:
        """
    ...
    async def _send_category_summary(self, ctx: commands.Context[awbot], category: str) -> None:
    ...
    async def _send_error_not_in_category(self, ctx: commands.Context[awbot], error_name: str, category: str) -> None:
    ...
    async def _send_error_not_found(self, ctx: commands.Context[awbot], error_name: str, category: str) -> None:
    ...
    @checks.has_pl(level=8)
    async def mock_test(self, ctx: commands.Context[awbot], *, error_type: str) -> None:
        """
