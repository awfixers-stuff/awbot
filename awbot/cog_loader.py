...
from awbot.utils.sentry import safe_set_name, span, start_span, transaction
...
class CogLoader(commands.Cog):
    ...
    async def _load_single_cog(self, path: Path) -> None:
        ...
        try:
            # Get the path relative to the awbot package
            relative_path = path.relative_to(Path(__file__).parent)

            # Convert path to module format (e.g., awbot.cogs.admin.dev)
            module = f"awbot.{str(relative_path).replace('/', '.').replace('\\', '.')[:-3]}"
