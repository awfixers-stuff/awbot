"""awbotApp: Orchestration and lifecycle management for the awbot Discord bot."""
...
async def get_prefix(bot: awbot, message: discord.Message) -> list[str]:
    ...
    if message.guild:
        try:
            from awbot.database.controllers import DatabaseController  # noqa: PLC0415
...
class awbotApp:
    """Orchestrates the startup, shutdown, and environment for the awbot bot."""

    def __init__(self):
        """Initialize the awbotApp with no bot instance yet."""
        self.bot = None

    def run(self) -> None:
        """Run the awbot bot application (entrypoint for CLI)."""
    ...
    async def start(self) -> None:
        """Start the awbot bot, handling setup, errors, and shutdown."""
        self.setup_sentry()
