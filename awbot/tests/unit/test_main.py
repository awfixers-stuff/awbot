...
# Mock the config loading before importing awbot.main to prevent FileNotFoundError in CI
...
class TestMain:
    ...
    @patch("awbot.main.awbotApp")
    def test_run_creates_app_and_calls_run(self, mock_awbot_app_class: Mock) -> None:
        """Test that run() creates a awbotApp instance and calls its run method."""
        # Arrange
        mock_app_instance = Mock()
        mock_awbot_app_class.return_value = mock_app_instance

        # Act
        awbot.main.run()

        # Assert
        mock_awbot_app_class.assert_called_once()
        mock_app_instance.run.assert_called_once()

    @patch("awbot.main.awbotApp")
    def test_run_propagates_app_exceptions(self, mock_awbot_app_class: Mock) -> None:
        """Test that run() propagates exceptions from awbotApp.run()."""
        # Arrange
        mock_app_instance = Mock()
    ...
    @patch("awbot.main.awbotApp")
    def test_run_propagates_app_creation_exceptions(self, mock_awbot_app_class: Mock) -> None:
        """Test that run() propagates exceptions from awbotApp instantiation."""
        # Arrange
        mock_awbot_app_class.side_effect = ValueError("App creation failed")

        # Act & Assert
        with pytest.raises(ValueError, match="App creation failed"):
            awbot.main.run()

    @patch("awbot.main.run")
...
class TestMainExecution:
    ...
    def test_module_has_main_guard(self) -> None:
        ...
        import awbot.main  # noqa: PLC0415

        # Get the source code of the main module
        source = inspect.getsource(awbot.main)
    ...
    @patch("awbot.main.awbotApp")
    def test_run_function_signature(self, mock_awbot_app_class: Mock) -> None:
        """Test that the run function has the correct signature."""

        # Check that run() takes no arguments
        sig = inspect.signature(awbot.main.run)
...
class TestMainIntegration:
    ...
    def test_import_has_no_side_effects(self) -> None:
        ...
        for _ in range(3):
            importlib.reload(awbot.main)

    @patch("awbot.main.awbotApp")
    def test_cli_integration_compatibility(self, mock_awbot_app_class: Mock) -> None:
        """Test that the main.run() function works correctly when called from CLI."""
        # This tests the actual usage pattern from awbot/cli/core.py
        mock_app_instance = Mock()
        mock_awbot_app_class.return_value = mock_app_instance

        # Simulate the CLI calling run() (from awbot.cli.core start command)
        from awbot.main import run  # noqa: PLC0415
    ...
    @patch("awbot.main.awbotApp")
    def test_multiple_run_calls_create_separate_apps(self, mock_awbot_app_class: Mock) -> None:
        """Test that multiple calls to run() create separate awbotApp instances."""
        ...
        # Each call should create a new awbotApp instance
        assert mock_awbot_app_class.call_count == 3
    ...
    def test_module_can_be_executed_as_script(self) -> None:
        ...
        # We mock the awbotApp to prevent the bot from starting

        # Create a temporary script that imports and patches awbotApp
        ...
        test_script = textwrap.dedent("""
            ...
            with patch("pathlib.Path.read_text") as mock_read_text:
                ...
                with patch("awbot.app.awbotApp") as mock_app:
                    mock_instance = Mock()
    ...
    def test_docstring_is_present_and_meaningful(self) -> None:
        ...
        assert awbot.main.__doc__ is not None
        assert len(awbot.main.__doc__.strip()) > 10
        assert "entrypoint" in awbot.main.__doc__.lower() or "entry point" in awbot.main.__doc__.lower()

        # Test that the run function also has a docstring
        assert awbot.main.run.__doc__ is not None
        assert len(awbot.main.run.__doc__.strip()) > 10
