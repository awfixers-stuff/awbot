"""Docker commands for the awbot CLI."""
...
from awbot.utils.env import is_dev_mode
...
RESOURCE_MAP = {
    ...
    "containers": {
        ...
        "regex": [r"^(awbot(-dev|-prod)?|memory-test|resource-test)$"],
        "remove": ["docker", "rm", "-f"],
    },
    "volumes": {
        "cmd": ["docker", "volume", "ls", "--format", "{{.Name}}"],
        "regex": [r"^awbot(_dev)?_(cache|temp)$"],
        "remove": ["docker", "volume", "rm", "-f"],
    },
    "networks": {
        "cmd": ["docker", "network", "ls", "--format", "{{.Name}}"],
        "regex": [r"^awbot_default$", r"^awbot-.*"],
...
def _get_service_name() -> str:
    ...
    return "awbot"  # Both dev and prod use the same service name
...
def _get_awbot_resources(resource_type: str) -> list[str]:
    """Get list of awbot-related Docker resources safely using data-driven approach."""
...
def _display_resource_summary(
    awbot_containers: list[str],
    awbot_images: list[str],
    awbot_volumes: list[str],
    awbot_networks: list[str],
) -> None:
    """Display summary of resources that will be cleaned up."""
    logger.info("awbot Resources Found for Cleanup:")
    logger.info("=" * 50)

    _log_resource_list("Containers", awbot_containers)
    _log_resource_list("Images", awbot_images)
    _log_resource_list("Volumes", awbot_volumes)
    _log_resource_list("Networks", awbot_networks)
...
def health() -> int:
    """Check health status of running awbot containers.

    Shows health check status for awbot services only.
    """
    try:
        # Get awbot container names
        awbot_containers = _get_awbot_resources("containers")

        if not awbot_containers:
            logger.info("No awbot containers found")
            return 0

        logger.info("awbot Container Health Status:")
        logger.info("=" * 60)

        for container in awbot_containers:
...
def cleanup(volumes: bool, force: bool, dry_run: bool) -> int:
    """Clean up awbot-related Docker resources (images, containers, networks).

    SAFETY: Only removes awbot-related resources, never affects other projects.
    """
    logger.info("Scanning for awbot-related Docker resources...")

    # Get awbot-specific resources
    awbot_containers = _get_awbot_resources("containers")
    awbot_images = _get_awbot_resources("images")
    awbot_volumes = _get_awbot_resources("volumes") if volumes else []
    awbot_networks = _get_awbot_resources("networks")
    ...
    if not any([awbot_containers, awbot_images, awbot_volumes, awbot_networks]):
        logger.info("No awbot-related Docker resources found to clean up")
        return 0

    # Show what will be removed
    _display_resource_summary(awbot_containers, awbot_images, awbot_volumes, awbot_networks)
    ...
    if not force:
        click.confirm("Remove these awbot-related Docker resources?", abort=True)

    logger.info("Cleaning up awbot-related Docker resources...")

    # Remove resources in order using data-driven approach
    _remove_resources("containers", awbot_containers)
    _remove_resources("images", awbot_images)
    _remove_resources("volumes", awbot_volumes)
    _remove_resources("networks", awbot_networks)

    logger.info("awbot Docker cleanup completed")
    return 0
...
def pull() -> int:
    """Pull the latest awbot images from the registry.

    Runs `docker compose pull` to update awbot images only.
    """
    cmd = [*_get_compose_base_cmd(), "pull"]
    logger.info("Pulling latest awbot Docker images")
    return run_command(cmd)
