...
from awbot.utils.functions import is_harmful, strip_formatting
    ...
    async def on_thread_create(self, thread: discord.Thread) -> None:
        ...
        if thread.parent_id == support_forum:
            ...
            if tags := [tag.name for tag in thread.applied_tags]:
                tag_list = ", ".join(tags)
                msg = f"<:awbot_notify:1274504953666474025> **New support thread created** - help is appreciated!\n{thread.mention} by {owner_mention}\n<:awbot_tag:1274504955163709525> **Tags**: `{tag_list}`"
