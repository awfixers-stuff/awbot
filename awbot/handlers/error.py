...
This module implements a centralized error handling mechanism for the awbot bot,
...
ContextOrInteraction = commands.Context[awbot] | discord.Interaction
...
# Note: Interaction is parameterized with the Bot type (awbot).
AppCommandErrorHandler = Callable[[discord.Interaction[awbot], app_commands.AppCommandError], Coroutine[Any, Any, None]]
...
ERROR_CONFIG_MAP: dict[type[Exception], ErrorHandlerConfig] = {
    ...
    # === Custom Errors (defined in awbot.utils.exceptions) ===
    ...
    # === Code Execution Errors (from awbot.utils.exceptions) ===
...
class ErrorHandler(commands.Cog):
    ...
    def __init__(self, bot: awbot) -> None:
        ...
        ----------
        bot : awbot
            The running instance of the awbot bot.
        """
        self.bot = bot
    ...
    async def _suggest_command(self, ctx: commands.Context[awbot]) -> list[str] | None:
        """
    ...
    async def _handle_command_not_found(self, ctx: commands.Context[awbot]) -> None:
        """
    ...
    async def on_command_error_listener(self, ctx: commands.Context[awbot], error: commands.CommandError) -> None:
        """
    ...
    async def on_app_command_error(
        self,
        interaction: discord.Interaction[awbot],
