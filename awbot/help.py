...
from awbot.ui.help_components import (
    BackButton,
    CategorySelectMenu,
    CloseButton,
    CommandSelectMenu,
...
from awbot.utils.help_utils import (
...
class awbotHelp(commands.HelpCommand):
    """
    Interactive help command for awbot.
    ...
    async def _create_main_embed(self) -> discord.Embed:
        ...
        """
        if CONFIG.BOT_NAME != "awbot":
            logger.info("Bot name is not awbot, using different help message.")
            embed = self._embed_base(
                "Hello! Welcome to the help command.",
                f"{CONFIG.BOT_NAME} is a self-hosted instance of awbot. The bot is written in Python using discord.py.\n\nIf you enjoy using {CONFIG.BOT_NAME}, consider contributing to the original project.",
            )
        else:
            embed = self._embed_base(
                "Hello! Welcome to the help command.",
    ...
    async def _add_bot_help_fields(self, embed: discord.Embed) -> None:
        ...
        bot_name_display = "awbot" if CONFIG.BOT_NAME == "awbot" else f"{CONFIG.BOT_NAME} (awbot)"
    ...
    def to_reference_list(
        ...
        def get_command_group(cmd: commands.Command[Any, Any, Any]) -> str | None:
            ...
            if cmd.cog:
                ...
                parts = module.split(".")
                # Assuming the structure is: awbot.cogs.<group>...
